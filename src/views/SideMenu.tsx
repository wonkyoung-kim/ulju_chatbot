...
const SideMenu = ({ isOpen, onClose, onTextScaleChange }) => {
    ...
    return (
        <>
            ...
            {/* 교육모드 팝업 */}
            <EducationPopup isOpen={isEducationPopupOpen} onClose={() => setIsEducationPopupOpen(false)} />
            {/* 주요일정 팝업 */}
            <SchedulePopup isOpen={isSchedulePopupOpen} onClose={() => setIsSchedulePopupOpen(false)} />
            {/* 요약본 다운로드 팝업 */}
            <SummaryDownloadPopup isOpen={isSummaryDownloadPopupOpen} onClose={() => setIsSummaryDownloadPopupOpen(false)} />
            {/* 사회재난 피해신고 팝업 */}
            <CalamityDownloadPopup isOpen={isCalamityDownloadPopupOpen} onClose={() => setIsCalamityDownloadPopupOpen(false)} />
        </>
    );
};
